...
import assignment_maze.MultiMazeProblem.MazeNode;
...
public class MultiMazeDriver extends Application {
	...
	// Split multi-robot path into separate animations for each robot
	private ArrayList<ArrayList<SearchNode>> splitPaths(List<SearchNode> allPaths, int k) {
		// initialize lists
		ArrayList<ArrayList<SearchNode>> separateLists = 
				new ArrayList<ArrayList<SearchNode>>();
		for (int i = 0; i < k; i++) {
			separateLists.add(new ArrayList<SearchNode>());
		}
		
		int pathIndex = 0; // the robot/path that this node belongs to
		for (int s = 0; s < allPaths.size(); s++) {
			separateLists.get(pathIndex).add(allPaths.get(s));
			pathIndex = (pathIndex + 1) % k;
		}
	...
	private void runSearches() {
		...
		int[] goalX = {2, 3, 4, 0};
		int[] goalY = {4, 0, 0, 0};
		MultiMazeProblem p = new MultiMazeProblem(maze, startX, startY, goalX, goalY, k);
		List<SearchNode> allPaths = p.astarSearch();
		
		ArrayList<ArrayList<SearchNode>> separatePaths = splitPaths(allPaths, k);
		
//		for (ArrayList<SearchNode> path : separatePaths) {
...
//		}
	...
	private class AnimationPath {
		...
		public AnimationPath(MazeView mazeView, List<SearchNode> path) {
			searchPath = path;
			MazeNode firstNode = (MazeNode) searchPath.get(0);
		...
		public void doNextMove() {
			...
			if (currentMove < searchPath.size() && animationDone) {
				MazeNode mazeNode = (MazeNode) searchPath
						.get(currentMove);
